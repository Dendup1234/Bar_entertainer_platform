...
export const sendOtp = async (req, res) => {
  try {
    // Request body
    const { stageName, name, phone, email, password } = req.body;
    ...
    const existingUser = await Entertainer.findOne({ email: normalizedEmail });
    if (existingUser)
...
export const verifyOtp = async (req, res) => {
  try {
    ...
    // already registered?
    const existing = await Entertainer.findOne({ email: normalizedEmail });
    if (existing)
      return res.status(409).json({ message: "Email already registered" });
    ...
    const user = await Entertainer.create({
      name: pending.name,
      email: pending.email,
      phone: pending.phone,
...
export const login = async (req, res) => {
  ...
  const user = await Entertainer.findOne({ email }).select("+password");
  if (!user) {
...
export const sendPasswordResetOtp = async (req, res) => {
  try {
    ...
    const user = await Entertainer.findOne({ email: normalizedEmail });
    if (!user)
...
export const setNewPassword = async (req, res) => {
  try {
    ...
    const user = await Entertainer.findOne({ email });
